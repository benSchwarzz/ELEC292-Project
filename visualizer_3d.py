...
# =============================================================================
# CONFIGURATION — edit CSV_PATH to point to any accelerometer CSV
# =============================================================================
CSV_PATH       = "Data/CSV/Data_Jumping_SweaterPocket_Sachin.csv"   # FIX: use local filename
STEP           = 500    # samples added per animation frame
...
COLOURS = ['r', 'g', 'b']
AXES    = ['X', 'Y', 'Z']
CMAP    = mplcm.get_cmap('plasma')  # FIX: use mplcm.get_cmap to avoid deprecation warning
...
acc  = df.iloc[:, 1:4].values.astype(np.float32)   # columns 1-3: x, y, z
N    = len(time)
...
for i, ax in enumerate(axes):
    ...
    ax.set_ylabel(f"{AXES[i]} (m/s²)", color=COLOURS[i],
...
fig.suptitle(
    f"Accelerometer — {CSV_PATH.split('/')[-1].split(chr(92))[-1]}",
    color=TEXT_C, fontsize=12, fontweight="bold", y=0.98,
)

# Fixed y-limits computed from the full dataset so axes don't jump during playback
for i, ax in enumerate(axes):
...
for spine in stat_ax.spines.values():
    spine.set_edgecolor(GRID_C)
stat_ax.set_title('LIVE STATS', color=TEXT_C, fontsize=9,
...
for i in range(3):
    base = i * 4
    stat_texts.append(
        stat_ax.text(0.5, y_pos[base], f"── {AXES[i]} axis ──",
...
ax3d.set_title("3D Trajectory", color=TEXT_C, fontsize=9,
...
def update(frame):
    ...
    # 3-D trail — oldest points dark, newest points bright
    ...
    scatter3d[0] = ax3d.scatter(
        x3, y3, z3,
        c=colours, s=4, alpha=0.7, depthshade=True,
    )
