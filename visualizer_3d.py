...
COLOURS = ['r', 'g', 'b']
AXES = ['X', 'Y', 'Z']
CMAP = get_cmap('plasma')   # colormap for 3D trail (oldest=dark, newest=bright)
...
# ── Figure layout ─────────────────────────────────────────────────────────────
#   Left col  : 3 stacked 2D subplots
#   Right col : stats panel (top) + 3D scatter (bottom, spans 2 rows)
...
fig.suptitle(
    f"Accelerometer — {CSV_PATH.split(chr(92))[-1]}",
    color=TEXT_C, fontsize=12, fontweight="bold", y=0.98,
)

# Fixed y-limits from full dataset so they never jump around
for i, ax in enumerate(axes):
...
# Fixed 3D limits from full dataset
ax3d.set_xlim(acc[:, 0].min(), acc[:, 0].max())
ax3d.set_ylim(acc[:, 1].min(), acc[:, 1].max())
ax3d.set_zlim(acc[:, 2].min(), acc[:, 2].max())

# Mutable container for the scatter object so update() can replace it each frame
scatter3d = [None]
...
def update(frame):
    ...
    for i, (ax, line) in enumerate(zip(axes, lines)):
        ...
        # Live stats using all data up to current frame
    ...
    # ── 3D scatter ────────────────────────────────────────────────────────────
    # Show only the last TRAIL_POINTS samples as a fading colour trail
    ...
    # Colour ramp: oldest point = dark end of colormap, newest = bright end
    n_trail = len(x3)
    colours = CMAP(np.linspace(0.15, 1.0, n_trail))

    # Scatter collections can't be updated in place so remove and redraw
